...
export class RegisterAdmin extends Component {
  ...
  onSubmit = async e => {
    ...
    if (_userType === "waiter") _userType = 2;
    if (_userType === "cashier") _userType = 3;
    if (_userType === "kitchen") _userType = 4;
    try {
      let regisUser = await axios.post(url + "/users", {
        userName: username,
        password: password,
        email: email,
        userType: _userType,
      ...
      if (regisUser.data.error) return alert(regisUser.data.error);
    this.username.value = '';
    this.password.value = '';
    this.name.value = '';
    this.email.value = '';
            ...
            <div className="card-body">
              ...
              <form className="form-group mb-0">
                ...
                <div className="card-title">
                  ...
                  <input
                    className="form-control"
                    type={this.state.cek ? "text" : "password"}
                    ref={input => {
                      this.password = input;
                    }}
                  />
                  <button
                    ...
                    onClick={this.cekPassword}
                  >
                    {this.state.cek ? "Hide Password" : "Show Password"}
                  </button>
                </div>
                <div className="card-title">
                  <h3>Name : </h3>
                ...
                <div className="card-title">
                  ...
                  >
                    ...
                    <DropdownMenu className="mr-2 w-100">
                      <DropdownItem
                        onFocus={() => this.setState({ userType: "Cashier" })}
                      >
                        Cashier
                      </DropdownItem>
                      <DropdownItem
                        onFocus={() => this.setState({ userType: "Kitchen" })}
                      >
                        Kitchen
                      </DropdownItem>
                      <DropdownItem
                        onFocus={() => this.setState({ userType: "Waiter" })}
                      >
                        Waiter
                      </DropdownItem>
